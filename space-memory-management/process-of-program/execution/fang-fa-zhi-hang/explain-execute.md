...
在字节码模块中提供了一个code\_at\(\)函数来对“取码”进行实现，该函数可以根据方法对象（methodOop）和bci进行取码
...
可以看到，根据模版生成了对应机器码之后generate\(_masm\),还会调用 \__ dispatch\_epilog\(tos\_out, step\)方法来生成跳转到下一条字节码对应机器码的入口地址，再看具体的实现：
